...
//$datafrom = date('YmdHis',strtotime(date('Ymd',strtotime("-1 days")))+1);
$dateto = date('YmdHis');
//$datato = date('YmdHis',strtotime(date('Ymd')));
$sequence =  date('YmdHis');
$pagesize = 100;
//$localurl = '/Users/lmjean/sites/2222/';//本地保存文件路径
			...
			if(count($dataarr) >0){
				foreach ($dataarr as $key => $value) {
					...
					if($value['game_file']['channel_id'] == '20032414'){
						...
						if($result){
							$str = "apptype=\"$apptype\",icon=\"$icon\",pkg=\"".$value['game_file']['package_name']."\",imgs=$imgs,version=\"".$value['game_file']['version_name']."\",versioncode=\"".$value['game_file']['version_code']
							...
							$insertsql = $pdo->exec("insert into $PRIFIX" ."app set $str");
						}else{
