...
class ResourceController extends Controller {
     ...
     * 单一app下载日志记录
     * 参数：
     * @param int userid:(必填) 厂区本地用户id
     * @param int appid:(必填) APP id
     * @param int factid:(选填) 厂区id
     */
    public function logappdownload(){
        $userid = I('post.uid','','int');
        $facid = I('post.facid','','int');
        if(!$facid){
            $facid = C('FACTORY_ID');
        }
        $appid = I('post.appid','','int');
        $ll = D('Log','Logic');
        $ll->addUseLog($appid,C('RES_TYPE_APP'),$userid,$facid);
    }
    /**
     * @Date 2016-05-08
     * @Author IceLight
     ...
     * 参数：
     * @param int userid:(必填) 厂区本地用户id
     * @param int videoid:(必填) Video id
     * @param int facid:(选填) 厂区id
     */
     public function logvideoplay(){
        $userid = I('post.uid','','int');
         $facid = I('post.facid','','int');
        if(!$facid){
            $facid = C('FACTORY_ID');
        }
        $videoid = I('post.videoid','','int');
        $ll = D('Log','Logic');
        $ll->addUseLog($videoid,C('RES_TYPE_VIDEO'),$userid,$facid);
    }
